use crate::{
    common::{app_state::AppState, entities::base::DbRepo, fs::file_utils::get_avatar_buffer},
    routes::{
        messages::message_route::{create_message, get_message, get_messages},
        profiles::profile_route::{create_profile, get_profile, get_profile_by_user},
    },
};
...
use fake::{
    faker::{
        ...
        internet::{en::DomainSuffix, en::Username},
    ...
    test::init_service(
        App::new().app_data(app_data.clone()).service(
            web::scope("/v1")
                ...
                .service(
                    web::resource("/profile/username/{user_name}")
                        .route(web::get().to(get_profile_by_user::<DbRepo>)),
                )
...
// NOTE: Creating a multipart form for testing purposes.
...
pub fn get_profile_create_multipart(
    ...
    payload
        .extend(format!("Content-Disposition: form-data; name=\"user_name\"\r\n\r\n").as_bytes());
    payload.extend(format!("{}\r\n", Username().fake::<String>()).as_bytes());
    payload.extend(format!("--{}\r\n", boundary).as_bytes());
    payload
        .extend(format!("Content-Disposition: form-data; name=\"full_name\"\r\n\r\n").as_bytes());
    payload.extend(
        format!(
            "{} {}\r\n",
    ...
    payload.extend(format!("--{}\r\n", boundary).as_bytes());
    payload
        .extend(format!("Content-Disposition: form-data; name=\"description\"\r\n\r\n").as_bytes());
    payload.extend(
        format!(
            "{}\r\n",
            Sentence(Range { start: 8, end: 10 }).fake::<String>()
        )
        .as_bytes(),
    );
    payload.extend(format!("--{}\r\n", boundary).as_bytes());
    ...
    if with_avatar == true {
        payload.extend(
            // NOTE: Putting `b` in front of a string will is the same as calling `.as_bytes`! - JW
